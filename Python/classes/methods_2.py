...
def Kepler(E, t, variables):
    ...
    if const.e != 0: 
        ...
        M = const.n*((t-variables.t_end)*const.dt) + M0
        Ex = M
        ...
        # if E != 0:
        # i += 1
        # Var.deltaM[0] = abs(M-Mx)
        # Var.deltaE[0] = abs(E-Ex)
        variables.E_end = E
        variables.Ex_end = Ex
        # Var.J[0] = j 
            
        "True anomaly tv"    
        tv = 2*np.arctan(np.sqrt((1+const.e)/(1-const.e))*(np.sin(E/2)/(np.cos(E/2))))
        variables.t_end = t
        r_Didymoon_r = const.a*(1-const.e*np.cos(E))    
        ...
        tv = const.omega*t
        r_Didymoon_r = const.a
    
    
    r_Didymoon_x = r_Didymoon_r*np.cos(tv)
    r_Didymoon_y = r_Didymoon_r*np.sin(tv)
    r_Didymoon_z = 0
    
    r_Didymoon_i = np.zeros(3, dtype=np.float128)
    r_Didymoon_i[0] = r_Didymoon_x
    r_Didymoon_i[1] = r_Didymoon_y
    r_Didymoon_i[2] = r_Didymoon_z
    
    r_Didymoon_r, r_Didymoon_theta, r_Didymoon_phi = cart_to_sph(r_Didymoon_x, r_Didymoon_y, r_Didymoon_z)

    return r_Didymoon_i, r_Didymoon_r, r_Didymoon_phi
...
def fun(tim, v_i, var):
    ...
    r_Didymoon_i, r_Didymoon_r, r_Didymoon_phi = Kepler(var.E_end, tim, var) #Kepler function gets called for each evaluation of fun
    
    var.r_Didymoon_i[0][var.tn] = r_Didymoon_i[0]
    var.r_Didymoon_i[1][var.tn] = r_Didymoon_i[1]
    var.r_Didymoon_i[2][var.tn] = r_Didymoon_i[2]
    var.r_Didymoon_r[var.tn] = r_Didymoon_r
    var.r_Didymoon_phi[var.tn] = r_Didymoon_phi
    
    for j in range(3):
        r_i[j] = v_i[j+3] 
        r_sec_i[j] = r_i[j]-r_Didymoon_i[j]
